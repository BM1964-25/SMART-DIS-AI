...
export async function POST(_request: Request, context: RouteContext) {
  ...
  if (!optionalEnv) {
    ...
    if (document?.status === "needs_ocr" || document?.analysisQuality === "needs_ocr") {
      return jsonError(
        "Fristenerkennung ist für dieses Dokument nicht belastbar, weil zuerst OCR erforderlich ist.",
        409
      );
    }

    const extractedText = document?.extraction?.extractedText ?? "";
    const deadlines = analyzeLocalDeadlines(
      documentId,
      `${document?.fileName ?? ""}\n${extractedText}`
    );
    await updateLocalDocumentFindings(documentId, { deadlines });
